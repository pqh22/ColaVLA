...
file_name = 'colavla_epoch10_msarplv2_ms6_seqformer_wckpt_fullcontext_regw80_pretraintraj_globalreason_top3pred'
...
model = dict(
    ...
    save_path=f'./results_planning/{file_name}/',   #save path for vlm models.
    use_grid_mask=True,
    frozen=False,
    use_lora=True,
    ...
    stage_index=[[5],[0,5],[0,2,5], [0,2,4,5], [0,1,2,4,5],[0,1,2,3,4,5]], # fine index
...
train_pipeline = [
    ...
    dict(type='LoadAnnoatationPUREQA', 
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            meta_keys=('pred_traj2', 'gt_planning', 'sample_idx', 'min_index', 'vlm_labels', 'ori_shape', 'img_shape','pad_shape', 'scale_factor', 'flip', 'box_mode_3d', 'box_type_3d', 'img_norm_cfg', 'scene_token'))
...
data = dict(
    ...
    workers_per_gpu=4, # origin 4 debug 0
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes3d_occ_ego_temporal_infos_train_classv3.pkl',
    ...
    val=dict(
        ...
        ann_file=data_root + 'nuscenes3d_occ_ego_temporal_infos_val_classv3.pkl',
        classes=class_names, 
        modality=input_modality),
    test=dict(
        type=dataset_type, 
...
optimizer_config = dict(type='Fp16OptimizerHook', loss_scale='dynamic', grad_clip=dict(max_norm=10, norm_type=2)) # origin 35
...
load_from='ckpts/merge_trajproj.pth'
resume_from=None
