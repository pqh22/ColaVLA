...
def custom_train_detector(
    ...
    if os.getenv("GRAD_DEBUG") == "1" and isinstance(
        cfg.get("optimizer_config", None), dict
    ):
        optimizer_cfg = cfg.optimizer_config.copy()
        if optimizer_cfg.get("type") == "Fp16OptimizerHook":
            optimizer_cfg["type"] = "Fp16OptimizerHookDebug"
            cfg.optimizer_config = optimizer_cfg
            logger.warning(
                "GRAD_DEBUG=1, optimizer hook switched to Fp16OptimizerHookDebug"
            )
        if os.getenv("TORCH_DISTRIBUTED_DEBUG") is None:
            os.environ["TORCH_DISTRIBUTED_DEBUG"] = "DETAIL"
            logger.warning("GRAD_DEBUG=1, set TORCH_DISTRIBUTED_DEBUG=DETAIL")
    ...
    if distributed:
        ...
        if cfg.get("static_graph", False):
            ...
            if torch.__version__ >= "2.1":
                extra_ddp_kwargs["static_graph"] = True
            ...
            if eval_model is not None:
                ...
                if hasattr(eval_model, "_set_static_graph"):
    ...
    if cfg.resume_from:
        runner.resume(
            cfg.resume_from, resume_optimizer=cfg.get("resume_optimizer", True)
        )
    elif cfg.load_from:
