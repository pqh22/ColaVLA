...
from projects.mmdet3d_plugin.models.utils.misc import MLN, topk_gather, transform_reference_points, memory_refresh, SELayer_Linear, transform_reference_points_lane
...
class PETRHeadImage(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRHeadImage):
        ...
        super(PETRHeadImage, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _init_layers(self):
        ...
        self.instance_embedding_lane = nn.Embedding(self.num_extra, self.embed_dims)
        
        self.query_embedding = nn.Embedding(self.num_extra, self.embed_dims)

        query_branch = []   
        for _ in range(1):
            query_branch.append(Linear(36, self.embed_dims))
            query_branch.append(nn.ReLU())
        query_branch.append(Linear(self.embed_dims, self.embed_dims))
        self.query_pos = nn.Sequential(*query_branch)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRHeadImage:
            convert_dict = {
    ...
    def forward(self, img_metas, pos_embed, **data):
        ...
        #1800, 256; 11, 256
        lane_embedding = self.instance_embedding_lane.weight #+ self.points_embedding_lane.weight.unsqueeze(0) 
        reference_points_lane = self.reference_points_lane(lane_embedding).sigmoid().unsqueeze(0).repeat(B, 1, 1)
        ...
        return vlm_memory, reference_points_lane
