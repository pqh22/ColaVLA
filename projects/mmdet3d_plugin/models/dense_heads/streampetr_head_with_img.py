...
class StreamPETRHeadWithImg(AnchorFreeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if class_weight is not None and (self.__class__ is StreamPETRHeadWithImg):
        ...
        super(StreamPETRHeadWithImg, self).__init__(
    ...
    def _init_layers(self):
        ...
        self.reference_points = nn.Embedding(self.num_query, 3)  # 600
        if self.num_propagated > 0:  # 300
        ...
        if self.use_commandemb:
            self.command_emb = nn.Embedding(3, self.output_dims)
    ...
    def pre_update_memory(self, data):
        ...
        if self.memory_embedding is None:
            ...
            self.memory_canbus = memory_refresh(
                self.memory_canbus[:, : self.can_bus_len], x
            )
            self.sample_time = data["timestamp"].new_zeros(B)
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        temp_pos = self.query_pos(
            nerf_positional_encoding(temp_reference_point.repeat(1, 1, self.n_control))
        )
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas):
        if self.training and self.with_dn:
            ...
            query_size = (
                pad_size + self.num_query + self.num_propagated + self.num_extra
            )
            ...
            if self.with_mask:
                temporal_attn_mask[
                    pad_size + self.num_extra :, pad_size : pad_size + self.num_extra
                ] = True
    ...
    def _load_from_state_dict(
        ...
        if (version is None or version < 2) and self.__class__ is StreamPETRHeadWithImg:
            convert_dict = {
    ...
    def forward(self, img_query, img_ref, img_metas, pos_embed, **data):
        """Forward function.
        Args:
        ...
        x = data["img_feats"]
        B, N, C, H, W = x.shape
        num_tokens = N * H * W  # 9600
        memory = x.permute(0, 1, 3, 4, 2).reshape(
            B, num_tokens, C
        )  # torch.Size([2, 9600, 256])
        ...
        query_pos = self.query_pos(
            nerf_positional_encoding(reference_points.repeat(1, 1, self.n_control))
        )
        tgt = torch.zeros_like(query_pos)  # torch.Size([2, 1554, 256]) 384 + 600 + 600
        ...
        if mask_dict and mask_dict["pad_size"] > 0:  # 270
            tgt[
                :, mask_dict["pad_size"] : mask_dict["pad_size"] + self.num_extra, :
            ] = img_query
            query_pos[
                :, mask_dict["pad_size"] : mask_dict["pad_size"] + self.num_extra, :
            ] = (
            ...
            tgt[:, : self.num_extra, :] = img_query
            query_pos[:, : self.num_extra, :] = query_pos[:, : self.num_extra, :] * 0
        # tgt torch.Size([2, 1554, 256])    memory torch.Size([2, 9600, 256])
        ...
        if mask_dict and mask_dict["pad_size"] > 0:
            reference_points = torch.cat(
                [
                    reference_points[:, : mask_dict["pad_size"], :],
                    reference_points[:, mask_dict["pad_size"] + self.num_extra :, :],
                ],
                dim=-2,
            )
        else:
        ...
        if self.output_dims is not None:
            if self.only_cat_query:
                ...
                vlm_memory = self.cat_projection(
                    vlm_memory.reshape(B, -1, self.output_dims)
                )
            else:
                vlm_memory = self.output_projection(vlm_memory)

            # if self.use_commandemb:
            #     can_bus_embed = self.can_bus_embed(can_bus_input) + self.command_emb(data['command'].view(-1).long()).reshape(B, -1)
            # else:
            #     can_bus_embed = self.can_bus_embed(can_bus_input)

            # if self.add_can_bus_embed:
            #     vlm_memory = vlm_memory + can_bus_embed.unsqueeze(-2)
            # else:
        ...
        #         if not os.path.exists(self.save_path_canbus):
        #             os.makedirs(self.save_path_canbus)
        #     except:
        #         pass
        #     for i in range(B):
        ...
        #     # Determine the offset to skip padding tokens
        #     if mask_dict and mask_dict['pad_size'] > 0:
        #         pad_size = mask_dict['pad_size']
        #         # Skip the padding tokens at the beginning
        #         valid_outs_dec = last_outs_dec[:, pad_size:, :]  # [B, num_queries - pad_size, dim]
        #     else:
        #         pad_size = 0
        #         valid_outs_dec = last_outs_dec  # [B, num_queries, dim]

        #     # Normalize for cosine similarity computation
        #     valid_outs_dec_norm = torch.nn.functional.normalize(valid_outs_dec, dim=-1)  # [B, num_valid_queries, dim]

        #     if align_with_ego:
        #         ego_norm = torch.nn.functional.normalize(can_bus_embed, dim=-1) # [B, 1, dim]
        #         # Compute cosine similarity: [B, num_valid_queries, 1]
        #         similarity = torch.matmul(valid_outs_dec_norm, ego_norm.transpose(1, 2))

        #         # Average similarity across all vlm_memory tokens: [B, num_valid_queries]
        #         avg_similarity = similarity.squeeze(-1) # [B, num_valid_queries]

        #     else:
        #         vlm_memory_norm = torch.nn.functional.normalize(vlm_memory, dim=-1)  # [B, num_extra, dim]

        #         # Compute cosine similarity: [B, num_valid_queries, num_extra]
        #         similarity = torch.matmul(valid_outs_dec_norm, vlm_memory_norm.transpose(1, 2))
        ...
        #     critical_tokens = torch.gather(valid_outs_dec, 1, topk_indices.unsqueeze(-1).expand(-1, -1, valid_outs_dec.size(-1)))
        #     # Adjust indices to account for skipped padding
        #     critical_indices = topk_indices + pad_size
        # Determine the offset to skip padding tokens
        last_outs_dec = outs_dec[-1]  # [B, num_queries, dim]
        if mask_dict and mask_dict["pad_size"] > 0:
            pad_size = mask_dict["pad_size"]
            # Skip the padding tokens at the beginning
            valid_outs_dec = last_outs_dec[
                :, pad_size:, :
            ]  # [B, num_queries - pad_size, dim]
        else:
            pad_size = 0
            valid_outs_dec = last_outs_dec  # [B, num_queries, dim]
        ...
        if mask_dict and mask_dict["pad_size"] > 0:
            ...
            outs = {
                ...
                "can_bus_embed": can_bus_embed.unsqueeze(-2)
                if self.add_can_bus_embed
                else None,
                "vlm_memory_256": vlm_memory_256,
                "rec_can_bus": can_bus_input,
                "valid_outs_dec": valid_outs_dec,
        ...
        return outs, vlm_memory, can_bus_embed
