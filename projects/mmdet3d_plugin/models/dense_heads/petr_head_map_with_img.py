...
class PETRHeadMapWithImg(AnchorFreeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if class_weight is not None and (self.__class__ is PETRHeadMapWithImg):
        ...
        self.critical_topk = critical_topk
        super(PETRHeadMapWithImg, self).__init__(
    ...
    def _init_layers(self):
        ...
        if self.output_dims is not None and not self.cat_memory:
            self.output_projection = nn.Linear(self.embed_dims, self.output_dims)

        if self.cat_memory:
            self.cat_projection = nn.Linear(self.output_dims, self.output_dims)
    ...
    def init_weights(self):
        ...
        xavier_init(self.reference_points_lane, distribution="uniform", bias=0.0)
        if self.loss_cls.use_sigmoid:
    ...
    def pre_update_memory(self, data):
        ...
        if self.memory_embedding is None:
            ...
            self.memory_reference_point = data["img_feats"].new_zeros(
                B, self.memory_len, self.n_control, 3
            )
            ...
            self.memory_reference_point = transform_reference_points_lane(
    ...
    def post_update_memory(
        self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec
    ):
        rec_reference_points = all_bbox_preds[-1].reshape(
            outs_dec.shape[1], -1, self.n_control, 3
        )
        ...
        self.memory_mask = torch.cat(
            [torch.ones_like(rec_timestamp), self.memory_mask], dim=1
        )
        self.memory_reference_point = transform_reference_points_lane(
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        temp_pos = self.query_pos(
            nerf_positional_encoding(temp_reference_point.flatten(-2))
        )
        ...
        if self.with_ego_pos:
            rec_ego_motion = torch.cat(
                [torch.zeros_like(tgt[..., :1]), rec_ego_pose[..., :3, :].flatten(-2)],
                dim=-1,
            )
            rec_ego_motion = nerf_positional_encoding(rec_ego_motion)
            memory_ego_motion = torch.cat(
                [self.memory_timestamp, self.memory_egopose[..., :3, :].flatten(-2)],
                dim=-1,
            ).float()
            memory_ego_motion = nerf_positional_encoding(memory_ego_motion)
            if os.getenv("LATENCY") == "1":
                memory_ego_motion = memory_ego_motion.to(torch.float16)
        ...
        if os.getenv("LATENCY") == "1":
            temp_pos += self.time_embedding(
                pos2posemb1d(self.memory_timestamp.to(torch.float16))
            )
        else:
            temp_pos += self.time_embedding(pos2posemb1d(self.memory_timestamp).float())

        if os.getenv("LATENCY") == "1":
            rec_ego_pose = rec_ego_pose.to(torch.float16)
    ...
    def _load_from_state_dict(
        ...
        if (version is None or version < 2) and self.__class__ is PETRHeadMapWithImg:
            convert_dict = {
    ...
    def forward(self, img_query, img_metas, pos_embed, **data):
        """Forward function.
        Args:
        ...
        if os.getenv("HEAD_DEBUG") == "1":
            ipdb.set_trace()
        # 1800, 256; 11, 256
        lane_embedding = self.instance_embedding_lane.weight.unsqueeze(
            -2
        ) + self.points_embedding_lane.weight.unsqueeze(0)
        ...
        query_embedding = img_query

        # attn mask for Hy
        self_attn_mask = (
            torch.zeros(
                [self.num_lane + self.num_extra, self.num_lane + self.num_extra]
            )
            .bool()
            .to(x.device)
        )
        self_attn_mask[
            self.num_lanes_one2one + self.num_extra :,
            0 : self.num_lanes_one2one + self.num_extra,
        ] = True
        self_attn_mask[
            0 : self.num_lanes_one2one + self.num_extra,
            self.num_lanes_one2one + self.num_extra :,
        ...
        temporal_attn_mask = (
            torch.zeros(
                [
                    self.num_lane + self.num_extra,
                    self.num_lane + self.num_extra + self.memory_len,
                ]
            )
            .bool()
            .to(x.device)
        )
        temporal_attn_mask[: self_attn_mask.size(0), : self_attn_mask.size(1)] = (
            self_attn_mask
        )
        if self.with_mask:
            temporal_attn_mask[self.num_extra :, : self.num_extra] = True

        tgt, query_pos, reference_points_lane, temp_memory, temp_pos, rec_ego_pose = (
            self.temporal_alignment(query_pos, tgt, reference_points_lane)
        )
        ...
        outs_dec = self.transformer(
            tgt, memory, query_pos, pos_embed, temporal_attn_mask, temp_memory, temp_pos
        )
        ...
        outs_dec = torch.nan_to_num(outs_dec)  # torch.Size([6, 2, 1800, 256])
        ...
        all_lane_preds[..., 0:3] = (
            all_lane_preds[..., 0:3] * (self.pc_range[3:6] - self.pc_range[0:3])
            + self.pc_range[0:3]
        )
        ...
        # outs_dec.shape torch.Size([6, 2, 1800, 256])
        # vlm_memory.shape torch.Size([2, 384, 256])

        # # Select critical tokens based on cosine similarity with vlm_memory
        # critical_tokens = None
        # critical_indices = None
        # if self.critical_topk != -1:
        #     # Use the last decoder layer output
        #     last_outs_dec = outs_dec[-1]  # [B, num_queries, dim]

        #     # Normalize for cosine similarity computation
        #     last_outs_dec_norm = torch.nn.functional.normalize(last_outs_dec, dim=-1)  # [B, num_queries, dim]

        #     if ego_embeds is not None:
        #         assert ego_embeds.shape[1] == 1, "ego_embeds should be a single embedding"
        #         ego_embeds_norm = torch.nn.functional.normalize(ego_embeds, dim=-1)  # [B, 1, dim]
        #         similarity = torch.matmul(last_outs_dec_norm, ego_embeds_norm.transpose(1, 2)) # [B, num_queries, 1]
        #         avg_similarity = similarity.squeeze(-1) # [B, num_queries]
        #     else:
        #         vlm_memory_norm = torch.nn.functional.normalize(vlm_memory, dim=-1)  # [B, num_extra, dim]

        #         # Compute cosine similarity: [B, num_queries, num_extra]
        #         similarity = torch.matmul(last_outs_dec_norm, vlm_memory_norm.transpose(1, 2))

        #         # Average similarity across all vlm_memory tokens: [B, num_queries]
        #         avg_similarity = similarity.mean(dim=-1)

        #     # Select top-k tokens based on similarity
        #     topk_values, topk_indices = torch.topk(avg_similarity, min(self.critical_topk, avg_similarity.size(-1)), dim=-1)

        #     # Gather critical tokens: [B, topk, dim]
        #     critical_tokens = torch.gather(last_outs_dec, 1, topk_indices.unsqueeze(-1).expand(-1, -1, last_outs_dec.size(-1)))
        #     critical_indices = topk_indices
        ...
        outs = {
            ...
            "vlm_memory_256": vlm_memory_256,
            "last_out_dec": outs_dec[-1],  # [B, num_queries, dim]
            # 'critical_tokens': critical_tokens,
            # 'critical_indices': critical_indices,
        }

        if self.output_dims is not None:
            if self.save_vlm_memory:
                try:
                ...
                for i in range(B):
                    np.save(
                        os.path.join(
                            self.save_path, "%s.npy" % img_metas[i]["sample_idx"]
                        ),
                        vlm_memory[i].cpu().numpy().astype(np.float16),
                    )
            if self.only_cat_query:
                vlm_memory = vlm_memory.reshape(B, -1, self.output_dims)
            elif self.cat_memory:
                vlm_memory = vlm_memory.reshape(B, -1, self.output_dims)
                vlm_memory = self.cat_projection(vlm_memory)
            else:
                vlm_memory = self.output_projection(vlm_memory)
        return outs, vlm_memory

    def loss(self, gt_lanes, preds_dicts, img_metas, gt_bboxes_ignore=None):
        """ "Loss function.
        Args:
        ...
        gt_labels = [
            torch.zeros(gt_lane.shape[0], dtype=torch.long, device=gt_lanes[0].device)
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_dir_i in zip(
            losses_cls[:-1], losses_bbox[:-1], losses_dir[:-1]
        ):
            loss_dict[f"d{num_dec_layer}.loss_cls_lane"] = loss_cls_i
            loss_dict[f"d{num_dec_layer}.loss_cls_H"] = (
                losses_cls_one2many[num_dec_layer] * self.lambda_one2many
            )
            loss_dict[f"d{num_dec_layer}.loss_bbox_lane"] = loss_bbox_i
            loss_dict[f"d{num_dec_layer}.loss_bbox_H"] = (
                losses_bbox_one2many[num_dec_layer] * self.lambda_one2many
            )
    ...
    def loss_single(
        ...
        pts_preds_dir = (
            bbox_preds.reshape(-1, self.n_control, 3)[:, 1:, :]
            - bbox_preds.reshape(-1, self.n_control, 3)[:, :-1, :]
        )
        pts_targets_dir = (
            bbox_targets.reshape(-1, self.n_control, 3)[:, 1:, :]
            - bbox_targets.reshape(-1, self.n_control, 3)[:, :-1, :]
        )
        loss_dir = self.loss_dir(
            pts_preds_dir, pts_targets_dir, dir_weights, avg_factor=num_total_pos
        )
        ...
        bbox_weights = (
            bbox_weights.mean(-1).unsqueeze(-1).repeat(1, bbox_preds.shape[-1])
        )
        # regression L1 loss
    ...
    def _get_target_single(
        ...
        # label targets
        labels = gt_bboxes.new_full((num_preds,), self.num_classes, dtype=torch.long)
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for img_id in range(len(img_metas)):
            ...
            boxes, scores = self._get_bboxes_single(
    ...
    def _get_bboxes_single(
        ...
        for p in range(self.n_control):
            ...
            det_bboxes[..., 3 * p + 1].clamp_(
                min=self.pc_range[1], max=self.pc_range[4]
            )
    ...
    def onnx_export(self, **kwargs):
        raise NotImplementedError(
            f"TODO: replace 4 with self.n_control : {self.n_control}"
        )
