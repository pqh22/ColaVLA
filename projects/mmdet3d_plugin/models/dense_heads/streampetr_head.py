...
class StreamPETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.code_weights = nn.Parameter(torch.tensor( # weight of 3d box position and direction
            self.code_weights), requires_grad=False)

        self.match_costs = nn.Parameter(torch.tensor( # weight of cls box iou ?
            self.match_costs), requires_grad=False)

        self.bbox_coder = build_bbox_coder(bbox_coder)

        self.pc_range = nn.Parameter(torch.tensor( # range of point cloud
    ...
    def _init_layers(self):
        ...
        self.query_embedding = nn.Embedding(self.num_extra, self.embed_dims) # 256 256
        
        if self.output_dims is not None:
            self.can_bus_embed = nn.Sequential(
                nn.Linear(74, self.embed_dims*4), # canbus + command + egopose
                nn.ReLU(),
                nn.Linear(self.embed_dims*4, self.output_dims), # 4096
            )
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        temp_reference_point = (self.memory_reference_point - self.pc_range[:3]) / (self.pc_range[3:6] - self.pc_range[0:3]) # norm and encode
    ...
    def forward(self, img_metas, pos_embed, **data):
        ...
        x = data['img_feats'] # torch.Size([2, 6, 1024, 40, 40])
        B, N, C, H, W = x.shape
        num_tokens = N * H * W
        memory = x.permute(0, 1, 3, 4, 2).reshape(B, num_tokens, C) # 2,9600,1024

        memory = self.input_projection(memory) # 2,9600,256
