...
class MultiHeadAttentionwDropout(nn.Module):
    def __init__(self, embed_dims, num_heads, dropout, flash_attn):
        ...
        if flash_attn:
            self.attn = FlashMHA(
                embed_dims, num_heads, dropout, dtype=torch.float16, device="cuda"
            )
        else:
    ...
    def forward(self, query, key, value, query_pos, key_pos, attn_mask):
...
class PETRTransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        # 2. LayerNorm
        self.transformer_layers.append(nn.LayerNorm(embed_dims))
        # 3. Multi-head Cross-attention
        self.transformer_layers.append(
        ...
        self.transformer_layers.append(nn.LayerNorm(embed_dims))
        # 5. Feed-forward Network
        self.transformer_layers.append(FFN(embed_dims, feedforward_dims, dropout))
        # 6. LayerNorm
        self.transformer_layers.append(nn.LayerNorm(embed_dims))

    def forward(
        self, query, key, query_pos, key_pos, attn_mask, temp_memory=None, temp_pos=None
    ):
        ...
        query, _ = self.transformer_layers[0](
            query, temp_key, temp_value, query_pos, temp_pos, attn_mask=attn_mask
        )
        # 2. LayerNorm
        query = self.transformer_layers[1](query)
        # 3. Multi-head Cross-attention (between queries and keys)
        query, _ = self.transformer_layers[2](
            query, key, key, query_pos, key_pos, attn_mask=None
        )
        # 4. LayerNorm
...
class ConstructiveTransformerLayer(nn.Module):
    ...
    def forward(
        ...
        # query = self.transformer_layers[1](query)
        # 3. Multi-head Cross-attention (between queries and keys)
        query, attn1 = self.transformer_layers[0](
            query, key, key, query_pos, key_pos, attn_mask=None
        )
        # 4. LayerNorm
        query = self.transformer_layers[1](query)
    ...
    def forward(
        ...
        for layer in self._layers:
            if self._with_cp and self.training:
                ...
                query = layer(
                    query, key, query_pos, key_pos, attn_mask, temp_memory, temp_pos
                )
