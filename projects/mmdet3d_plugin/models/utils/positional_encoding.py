...
def gen_sineembed_for_position(pos_tensor, hidden_dim=256):
    ...
    scale = 2 * math.pi
    if os.getenv("LATENCY") == "1": dtype = torch.float16
    else: dtype = torch.float32
    dim_t = torch.arange(half_hidden_dim, dtype=dtype, device=pos_tensor.device)
...
def pos2posemb3d(pos, num_pos_feats=128, temperature=10000):
    ...
    dim_t = torch.arange(num_pos_feats, dtype=dtype, device=pos.device)
