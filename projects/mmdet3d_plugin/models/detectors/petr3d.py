...
class Petr3D(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img):
        ...
        if img is not None: # torch.Size([2, 6, 3, 640, 640])
            if img.dim() == 6:
                img = img.flatten(1, 2)
            if img.dim() == 5 and img.size(0) == 1:
                ...
                B, N, C, H, W = img.size()
                img = img.reshape(B * N, C, H, W) # torch.Size([12, 3, 640, 640])
            if self.use_grid_mask:
                img = self.grid_mask(img)

            img_feats = self.img_backbone(img) # list len=1 torch.Size([12, 1024, 40, 40])
        ...
        return img_feats_reshaped # torch.Size([2, 6, 1024, 40, 40])
    ...
    def prepare_location(self, img_metas, **data):
        pad_h, pad_w, _ = img_metas[0]['pad_shape'][0] # 640, 640, 3
        bs, n = data['img_feats'].shape[:2] # 2, 6
        x = data['img_feats'].flatten(0, 1) # torch.Size([12, 1024, 40, 40])
        location = locations(x, self.stride, pad_h, pad_w)[None].repeat(bs*n, 1, 1, 1) # torch.Size([12, 40, 40, 2]) self.stride=16
        return location # 0.0125 - 0.9875 0.0125 - 0.9875
    ...
    def position_embeding(self, data, memory_centers, img_metas):
        eps = 1e-5
        BN, H, W, _ = memory_centers.shape # 12, 40, 40, 2
        B = data['intrinsics'].size(0) # torch.Size([2, 6, 4, 4])
        ...
        D = self.coords_d.shape[0] # torch.Size([64])

        memory_centers = memory_centers.detach().view(B, LEN, 1, 2) # torch.Size([2, 9600, 1, 2])
        topk_centers = memory_centers.repeat(1, 1, D, 1) # torch.Size([2, 9600, 64, 2])
        coords_d = self.coords_d.view(1, 1, D, 1).repeat(B, num_sample_tokens, 1 , 1) # torch.Size([2, 9600, 64, 1])
        coords = torch.cat([topk_centers, coords_d], dim=-1) # torch.Size([2, 9600, 64, 3])
    ...
    def forward_pts_train(self,
                          ...
                          vlm_attn_mask,
                          lane_pts,
                          **data): # dict_keys(['img', 'prev_exists', 'lidar2img', 'intrinsics', 'extrinsics', 'timestamp', 'img_timestamp', 'ego_pose', 'ego_pose_inv', 'command', 'can_bus', 'img_feats'])
        ...
        pos_embed = self.position_embeding(data, location, img_metas) # torch.Size([2, 9600, 256])
        losses = dict()
        ...
        if self.with_lm_head:
            vision_embeded = torch.cat([vision_embeded_obj, vision_embeded_map], dim=1) # torch.Size([2, 513, 4096])
            ...
            losses.update(vlm_loss=vlm_loss[0]) # odict_keys(['loss', 'logits']) tensor(1.2431, device='cuda:0', torch.Size([2, 2313, 32000])
            
        if self.with_img_roi_head: # False
    ...
    def forward_train(self,
        ...
        if self.tokenizer is not None:
            ...
            input_ids = torch.nn.utils.rnn.pad_sequence( # torch.Size([2, 1801])
                input_ids, # list torch.Tensor len=B size=[1554] [1801]
                batch_first=True,
                padding_value=self.tokenizer.pad_token_id)
            
            vlm_labels = torch.nn.utils.rnn.pad_sequence(vlm_labels, # torch.Size([2, 1801])
                                                    batch_first=True,
                                                    padding_value=IGNORE_INDEX)
            
            input_ids = input_ids[:, :self.tokenizer.model_max_length] # max=2048
            vlm_labels = vlm_labels[:, :self.tokenizer.model_max_length]
            vlm_attn_mask = input_ids.ne(self.tokenizer.pad_token_id) # padding is False torch.Size([2, 1801])
        else:
            input_ids = None
            vlm_labels = None
            vlm_attn_mask = None
