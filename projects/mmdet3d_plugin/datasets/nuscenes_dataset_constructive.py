...
class CustomNuScenesDatasetConstructive(NuScenesDataset):
    r"""NuScenes Dataset.
    ...
    def __init__(self, 
                 ...
                 eval_mode=['lane', 'det'],
                 constractive_index_path='/nfs/dataset-ofs-voyager-research/xschen/repos/SparseDrive/trajectory_samples_2048.pkl',
                 *args, 
                 **kwargs):
        ...
        self.constractive_index_path = constractive_index_path
        self.constractive_index = pickle.load(open(self.constractive_index_path, 'rb'))
    ...
    def get_data_info(self, idx):
        """Get data info according to the given list of indices.

        Args:
            indices (list[int]): List of indices of the sample data to get.

        Returns:
            dict: Data information that will be passed to the data \
                preprocessing pipelines. Each key contains a list of values corresponding to each index.
                It includes the following keys:

                - sample_idx (list[str]): Sample indices.
                - pts_filename (list[str]): Filenames of point clouds.
                - sweeps (list[list[dict]]): Infos of sweeps for each sample.
                - timestamp (list[float]): Sample timestamps.
                - img_filename (list[list[str]], optional): Image filenames for each sample.
                - lidar2img (list[list[np.ndarray]], optional): Transformations from lidar to different cameras for each sample.
                - ann_info (list[dict]): Annotation info for each sample.
        """
        input_dict = {
            'can_bus': [],
            'command': [],
        ...
        positive_samples = self.constractive_index[idx]['positive_samples']
        negative_samples = self.constractive_index[idx]['negative_samples']
        positive_idx = random.choice(positive_samples) if len(positive_samples) > 0 else idx
        negative_idx = random.choice(negative_samples) if len(negative_samples) > 0 else idx
        indices = [idx, positive_idx, negative_idx]
        for i, index in enumerate(indices):
            ...
            if self.modality['use_camera']:
                ...
                input_dict['img_timestamp'].extend(img_timestamp)
                input_dict['img_filename'].extend(image_paths)
                input_dict['lidar2img'].extend(lidar2img_rts)
                input_dict['intrinsics'].extend(intrinsics)
                input_dict['extrinsics'].extend(extrinsics)
                input_dict['prev_exists'].append(prev_exists)

            if not self.test_mode and i == 0:
                annos = self.get_ann_info(index)
                if 'lane_info' not in info.keys():
                    lane_pts = []
                else:
