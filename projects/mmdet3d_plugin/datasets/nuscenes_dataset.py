...
class CustomNuScenesDataset(NuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        if not self.test_mode:
            ...
            if 'gt_fut_traj' in info.keys():
                input_dict.update(
                    dict(
                        gt_fut_traj=info['gt_fut_traj'],
                        gt_fut_traj_mask=info['gt_fut_traj_mask'],
                        gt_fut_yaw=info['gt_fut_yaw'],
                        gt_fut_idx=info['gt_fut_idx'],
                    )
                )

            if 'gt_velocity' in info.keys():
                input_dict.update(
                    dict(
            ...
            if 'ego2global_translation' in info.keys():
                input_dict.update(
                    dict(
        ...
        if 'direction_category' in info.keys() and 'distance_category' in info.keys():
            input_dict.update(
                dict(
                    direction_category=info['direction_category'],
                    distance_category=info['distance_category'],
                )
            )

        if 'ego_labels' in info.keys():
            input_dict.update(
                dict(
                    ego_labels=info['ego_labels'],
                )
            )

        if 'canbus_stats' in info.keys():
            input_dict.update(
                dict(
                    canbus_stats=info['canbus_stats'],
                )
            )
        ...
        if 'path_label_v3' in info.keys():
            input_dict.update(
                dict(
                    path_label_v3=info['path_label_v3'],
                )
            )

        if 'speed_label_v3' in info.keys():
            input_dict.update(
                dict(
                    speed_label_v3=info['speed_label_v3'],
                )
            )

        if 'combined_label_v3' in info.keys():
            input_dict.update(
                dict(
                    combined_label_v3=info['combined_label_v3'],
                )
            )

        if 'combined_id_v3' in info.keys():
            input_dict.update(
                dict(
                    combined_id_v3=info['combined_id_v3'],
                )
            )
